...
pedigree_tt <- inner_join(pedigree, pedigree, by = c("o" = "p",
                                                     "p" = "o"),
                          keep = F)
pedigree_tt # NO CYCLICAL RELATIONSHIPS

# made it tall so that each parent relationship was its own row with parent_lod_score
filtered_franz <- pedigree %>%
  rename(Tree=o, parent_id=p)
  # MIGHT STILL WANT TO DOUBLE CHECK USING THE LOD SCORE VALUES
...
franz_coords <- inner_join(filtered_franz, convenient_parents_to_join, by="parent_id") %>%
  ...
  filter(!duplicated(Tree, parent_id)) # filter out duplicates in case of selfing
...
trees_distances %>%
  ...
  theme_classic() +
  ggtitle('500 candidates')

# filter out the super big one
trees_distances %>%
  filter(distance < 10000, distance != 0) %>%
  ggplot(aes(x = distance,y=after_stat(density))) +
  geom_histogram(fill="thistle2", bins=50) +
  geom_density(color = "red") +
  theme_classic()
