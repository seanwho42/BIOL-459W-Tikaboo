/*
 * $Id: prob.c 1734 2009-07-11 12:30:59Z markus $
 *
...
{
    int     i;
    double  sum_l = 0., log_c_f, log_c_m, log_c_both;
    ...
    for (i = 0; i < Probs.num_posteriors[id]; i++) {
        ...
        if (1 && Data.num_ramets - Data.num_samples > 0) {
                   ...
                   int outformat)
{
    ...
    for (i = 0; i < Data.num_samples; i++) {
        ...
        for (j = 0; j < Probs.num_posteriors[i]; j++) {
            ...
            if (j == 0 || outformat == OUTFORMAT_DETAIL) {      /* sorted, so first one is the most-likeliest */
                assigned = true;
                cnt_common = 0;
