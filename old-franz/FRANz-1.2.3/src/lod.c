/*
 * $Id: lod.c 1732 2009-07-10 18:45:54Z markus $
 * 
...
LODinit()
{
    ...
    MAKE1DDOUBLE(epsilon3, Data.num_loci);
    ...
    for (i = 0; i < Data.num_loci; i++) {
        ...
        epsilon2[i] =
            3 * Data.TypingError[i] * Data.TypingError[i] * (1. -
                                                             ...
                                                             TypingError[i]);
        epsilon3[i] =
            Data.TypingError[i] * Data.TypingError[i] * Data.TypingError[i];
...
{    
    ...
    if (child[0] < 0 || child[1] < 0) {
        ...
        if (p1[0] < 0)
            p = FREQ_ALLELE_PROB(locus,
                                    child[im]) + FREQ_CMP_ALLELES(child[im],
                                                                p1[1]);
        else
            p = FREQ_CMP_ALLELES(child[im],
                                    p1[0]) + FREQ_CMP_ALLELES(child[im],
                                                            p1[1]);
        ...
        if (p2[0] < 0)
            ...
            p += FREQ_CMP_ALLELES(child[im],
                                    p2[0]) + FREQ_CMP_ALLELES(child[im],
                                                            p2[1]);
        ...
        case 1:
            if (child[0] == child[1]) {
                ...
                return 0.5 * FREQ_ALLELE_PROB(locus, child[1]) +
                    ps[1] * 0.5 * FREQ_ALLELE_PROB(locus, child[0]);
            }
...
calcTransDyadMissing(int child[], int parent[], int locus)
{
    ...
    if (child[0] < 0 || child[1] < 0) {
        ...
        p = 0.5 * FREQ_ALLELE_PROB(locus, child[im]) +
            0.25 * 
                    (FREQ_CMP_ALLELES(child[im], parent[0]) +
                    FREQ_CMP_ALLELES(child[im], parent[1]));
        return p;
    }
...
calcPchildTripleLocus(int locus, int **child, int **p1, int **p2, double tpt)
{
    ...
    return log(epsilon0[locus] * tpt + epsilon1[locus] * (tpd1 + tpd2 + pg) +
               epsilon2[locus] * pg + epsilon3[locus] * pg);
}
...
calcDenominatorTripleMotherKnownLocus(int locus, int **genotype, int **mother)
{
    ...
    return log(epsilon0[locus] * tp + epsilon1[locus] * (tp + 2 * gp) +
               epsilon2[locus] * gp + epsilon3[locus] * gp);
}
