/*
 * $Id: mcmc.c 1732 2009-07-10 18:45:54Z markus $
 *
...
performStepCandidateParents(MCMC_STATE * state)
{
    state->x_f =
        state->sr_f +
        ((4. - state->sr_f) / D_MCMCCPGRID * RANDINT(D_MCMCCPGRID));
    if (!Data.has_sex_data) {
        ...
        state->x_m =
            state->sr_m +
            ((4. - state->sr_m) / D_MCMCCPGRID * RANDINT(D_MCMCCPGRID));

    assert(state->x_f >= state->sr_f && state->x_f <= 4.);
    assert(state->x_m >= state->sr_m && state->x_m <= 4.);
    PEDIGREEestmateSamplingRate(state->D, &state->sr_f, &state->sr_m);
    calcNfm(state);
    recalcPP(state);
}
...
gibbsChangeGenotype(MCMC_STATE * state, LOCUS_COORD lc)
{
    ...
    double  p_sum = 0., rd, max = MINUSINF;
    ...
    for (i = 0; i < cnt; i++) {
        ...
        state->_p_gibbs_genotype[i] -= max;
        state->_p_gibbs_genotype[i] = exp(state->_p_gibbs_genotype[i]);
        /* the cumulative ratio */
        p_sum += state->_p_gibbs_genotype[i];
        //      if (lc.locus == 3) if (lc.both) fprintf(stderr, "GIBBS %i ID: %i Locus: %i p: %f %f\n", i, lc.id, lc.locus, state->_p_gibbs_genotype[i], p_sum);
        state->_p_gibbs_genotype[i] = p_sum;
                ...
                bool(*accfp) (double, double, double,double))
{
    ...
    r1 = state->iter % (mcmc->_num_to_change + 2);
...
dumpMHparams(MCMC * mcmc)
{
    ...
    if (Options.Selfing) {
        fprintf(out, "p,Nf,Nm,samplingratef,samplingratem,selfing\n");
        ...
        fprintf(out, "p,Nf,Nm,samplingratef,samplingratem\n");
        for (i = 0; i < Probs.mh_sampled_pedigrees; i++)
            fprintf(out, "%f,%f,%f,%f,%f\n", mcmc->mh_p[i], mcmc->mh_Nf[i],
...
#pragma omp parallel for
    ...
    startSimulatedAnnealing(mcmc, states);
