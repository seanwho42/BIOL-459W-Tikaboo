/*
 * $Id: null.c 1557 2009-05-19 12:17:26Z markus $
 *
...
NULLdump(FILE * fp, NULLTEST nt)
{
    ...
    fprintf(fp, " +--------+--------+------------+------------+------------+--------------------------+\n");
    fprintf(fp, " | %6s | %6s | %10s | %10s | %10s | %24s |\n", "Allele", "Count",
            "Heterozyg.", "Homozyg.", "Frequency",
    ...
    for (i = 0;i < FREQ_RANGE(nt->locus); i++) {
        ...
        fprintf(fp, " | %6i | %6i | %10i | %10i | %10.5f | %24.5f |\n",
                Data.allele_frequencies[nt->locus].min + i, nt->ni[allele_id],
                nt->sum_nij[allele_id], nt->nii[allele_id],
                Data.allele_frequencies[nt->locus].freqs[i],
                nt->p_i_it[allele_id]);
